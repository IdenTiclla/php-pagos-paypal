...
class rpt_listaventas extends PDF_MC_Table
{
   ...
   function GenerarReporte()
   {
      ...
      $sql = "select dv.producto_id,
               ...
               dv.precio,
               dv.cantidad
            FROM detalleventas dv inner join productos p on dv.producto_id = p.id
            where dv.venta_id = $idventa
       ";
      $datos = $this->cnx->execute($sql);
      //el tamano max. ancho de la tabla es 190
      parent::imprimirCabezaTabla(
         array("#", "Id", "Producto", "P.U.($)", "Cant.", "Total $."),
         array(15, 25, 90, 20, 20, 20)
      );
      ...
      while ($reg = $this->cnx->next($datos)) {
         ...
         $subtotal = $reg[2] * $reg[3];
         $total = $total + $subtotal;

         parent::stContenido();
         parent::SetWidths(array(15, 25, 90, 20, 20, 20));
         parent::SetAligns(array('L', 'L', 'L', 'R', 'R', 'R'));
         @parent::Row(
            array(
               utf8_decode($nrofila),
               utf8_decode($reg[0]),
